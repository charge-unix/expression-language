...
test('short circuit evaluate', () => {
  ...
  let shortCircuits = [['false && object.foo()', {
    object: obj
  }, false], ['false and object.foo()', {
    object: obj
  }, false], ['true || object.foo()', {
    object: obj
  }, true], ['true or object.foo()', {
    object: obj
  }, true]];

  for (let shortCircuit of shortCircuits) {
    //console.log("Testing: ", shortCircuit[0]);
    let exprLang = new _ExpressionLanguage.default();
    expect(exprLang.evaluate(shortCircuit[0], shortCircuit[1])).toBe(shortCircuit[2]);
  }
});
test('short circuit compile', () => {
  let shortCircuits = [['false && foo', [{
    foo: 'foo'
  }], false], ['false and foo', [{
    foo: 'foo'
  }], false], ['true || foo', [{
...
test('caching for overridden variable names', () => {
  let expressionLanguage = new _ExpressionLanguage.default(),
      expression = 'a + b';
...
test('bad callable', () => {
  try {
    let expressionLanguage = new _ExpressionLanguage.default();
    expressionLanguage.evaluate("foo.myfunction()", {
      foo: {}
    });
    console.log("Shouldn't get to this point.");
    expect(true).toBe(false);
...
function getRegisterCallbacks() {
  let provider = {
    getFunctions: () => {
      return [new _ExpressionFunction.default('fn', () => {}, () => {})];
    }
  };
  return [[expressionLanguage => {
    expressionLanguage.register('fn', () => {}, () => {});
  }], [expressionLanguage => {
    expressionLanguage.addFunction(new _ExpressionFunction.default('fn', () => {}, () => {}));
  }], [expressionLanguage => {
...
function getEvaluateData() {
  ...
  false, // Provider
  null], ['foo.getFirst() + bar.getSecond()', {
    foo: {
      getFirst: () => {
        return 7;
      }
  ...
  }, 107, null], ['(foo.getFirst() + bar.getSecond()) / foo.second', {
    foo: {
      second: 4,
      getFirst: () => {
        return 7;
  ...
  }, 4, null], ['foo.getFirst() + bar.getSecond() / foo.second', {
    foo: {
      second: 4,
      getFirst: () => {
        return 7;
  ...
  }, 9, null], ['(foo.getFirst() + bar.getSecond() / foo.second) + bar.first[3]', {
    foo: {
      getFirst: () => {
        return 7;
      },
  ...
  }, "bar two", null], ['a[2] === "three" and b.myMethod(a[1]) === "bar two" and (b.myProperty == "foo" or b["myProperty"] == "foo") and b["property with spaces and &*()*%$##@% characters"] == "fun"', {
    a: ["one", "two", "three"],
    b: {
      myProperty: "foo",
      myMethod: word => {
    ...
    a: "Dogs",
    b: ["Cats", "Dogs"]
  }, true, null], ['a in outputs["typesOfAnimalsAllowed"]', {
    a: "Dogs",
    outputs: {
      typesOfAnimalsAllowed: ["Dogs", "Other"]
    }
  }, true, null], ['"Other" in inputs["typesOfAnimalsAllowed"]', {
    inputs: {
      typesOfAnimalsAllowed: ["Dogs", "Other"]
    }
  }, true], ['a not in b', {
